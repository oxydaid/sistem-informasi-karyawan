...
@if(in_array($role, ['super_admin', 'manager', 'hrd']))
    <div class="space-y-1 pt-2">
        ...
        <!-- Absensi Karyawan (eBilling Sync) -->
        <a href="{{ route('admin.attendances') }}" class="group flex items-center px-4 py-3 text-sm font-semibold rounded-2xl transition duration-150 {{ $isActive(['admin.attendances']) }}">
            <svg class="mr-3 h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
...
@if($role === 'employee')
    <div class="space-y-1">
        ...
        <a href="{{ route('employee.kpi') }}" class="group flex items-center px-4 py-3 text-sm font-semibold rounded-2xl transition duration-150 {{ $isActive(['employee.kpi']) }}">
            <svg class="mr-3 h-5 w-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
